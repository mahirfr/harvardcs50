...
void add_pairs(void);
int get_margin(pair pair);
void sort_pairs(void);
bool is_cycle(int candidate_1, int candidate_2);
...
void sort_pairs(void)
{
    for (int i = 0; i < pair_count; i++)
    {
        ...
        {
            if (get_margin(pairs[j]) > get_margin(pairs[i]))
            {
...
bool is_cycle(int candidate_1, int candidate_2)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        if(locked[candidate_2][i])
        {
            if (is_cycle(candidate_1, i))
                return true;
        }
    }
    return false;
